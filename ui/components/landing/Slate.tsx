import type { SlateCard } from "@/lib/pick-view";

type Props = { games?: readonly SlateCard[] };

export function Slate({ games }: Props = {}) {
  const cards: readonly SlateCard[] = games ?? [];
        ...
        <div className="slate-track">
          {cards.length === 0 ? (
            <article className="slate-card">
              <div className="sc-time">LIVE DB</div>
              ...
              <div className="sc-top">
                Run refresh picks to populate today&apos;s games.
              </div>
            </article>
          ) : (
            cards.map((card, i) => (
