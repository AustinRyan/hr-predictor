...
- **Model artifact deployment is still local-file based.** `registry/PRODUCTION` and `registry/v20260510_163447/` are gitignored, so this machine can refresh with the promoted full-game model, but another machine/CI runner would need the artifact copied or an artifact-store workflow before it can refresh with the same version.
