...
**Updated:** 2026-05-10 — bullpen/full-game probability branch
...
- **Bullpen-only historical rollout + candidate model (2026-05-10):** `src.features.builder --team-bullpen-only` repairs existing `opp_team_id`/`opp_bp_*` columns without rebuilding all feature families. Local Docker historical backfill updated 668,100 rows in 12.3 min; 100% now have `opp_team_id`, 662,185 have prior bullpen rates. Candidate full-game artifact `v20260510_163447` trained on the populated data and beats current starter artifact on full-game test labels: log_loss 0.3426 vs 0.3654, brier 0.0985 vs 0.1023, precision@top20 0.2237 vs 0.1976. Not promoted yet.
...
- **Current production artifact is still starter-matchup only.** Candidate full-game artifact `v20260510_163447` beat `v20260423_231941` in local full-game-label evaluation, but `registry/PRODUCTION` remains `v20260423_231941` until the candidate is explicitly promoted and a current-slate refresh is run.
