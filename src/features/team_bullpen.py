...
_LAST_3D = (
    "sp.game_date >= mk.reference_date - INTERVAL '3 days' AND sp.game_date < mk.reference_date"
)
...
def team_bullpen_sql() -> str:
    ...
    LEFT JOIN relief_pitches sp
      ON sp.pitcher_team_id = mk.opp_team_id
     AND sp.game_date < mk.reference_date
     AND sp.game_date >= DATE_TRUNC('year', mk.reference_date)::date
    GROUP BY mk.game_pk, mk.batter_id, mk.pitcher_id, mk.reference_date, mk.opp_team_id
    """
